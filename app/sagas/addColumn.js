...
function * ranking({mode}) {
	yield put(addColumn(ENDPOINT.RANKING, {mode}, `${RANKING.ILLUST_RANKING[mode]}ランキング`, THREE_HOUR))
}

function * r18Ranking({mode}) {
	yield put(addColumn(ENDPOINT.RANKING, {mode}, `${RANKING.ILLUST_R18_RANKING[mode]}ランキング`, THREE_HOUR))
}

function * userIllust({userId, name, account}) {
	yield put(addColumn(ENDPOINT.USER_ILLUSTS, {userId}, `${name}(${account})`, THREE_HOUR))
}
